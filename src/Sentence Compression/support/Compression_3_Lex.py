# -*- coding: utf-8 -*-
"""
Created on Tue Mar 28 20:08:34 2017
...
def countStats(textDir):
    ...
    word_children = defaultdict(lambda: defaultdict(str))
    for fil in textDir:
        ...
        for parse in readParses(ff):
            ...
            for (word, pos) in leaves:
                ...
                word_children[dire + fil][node + "-" + str(count)] = parse.get_children(node)
            ...
            #break
            count += 1
    return termCounts, word_depths, word_heads, word_children
...
termCounts, word_depths, word_heads, word_children  = countStats(textDir)
s_pred_out_file = open("E:\\SP 17\\5525 SLP\\Project\\Sentence Compression\\support\\Results\\Lex_Compressed_3.txt", "w+")
...
for fil in num:
    ...
    for parse in readParses(open(fi, encoding="utf-8")):
        ...
        problem = LpProblem("Sentence Compression 3", LpMaximize)
        varTab = {}
        ...
        for obj, wt in word_objects.items():
            ...
            obj_children = word_children[fi][obj]
            #print(obj, obj_children)
            for index, children in enumerate(obj_children):
                ((child, child_pos), child_dptag) = children
                if child_dptag in keep_dependency_archs :
                    problem += varTab[child+"-"+str(child_pos)+"-"+str(count)] >= varTab[obj]
